...
export const ERROR_MESSAGES = {
  ...
  NETWORK: {
    ...
    HTTP_ERROR: {
      user: (context: ErrorContext) => {
        ...
        return statusMessages[context.statusCode || 500] || 
               `Service error (${context.statusCode}). Please try again.`;
      },
      technical: (context: ErrorContext) => 
        `HTTP ${context.statusCode} error`
    }
  },
  ...
  VALIDATION: {
    ...
    TOO_SHORT: {
      ...
      technical: (context: ErrorContext) => 
        `Field too short: ${context.field}`
    },
    TOO_LONG: {
      user: (context: ErrorContext) => 
        `${context.field || 'Input'} is too long. Please shorten it.`,
      technical: (context: ErrorContext) => 
        `Field too long: ${context.field}`
    },
    INVALID_CHARACTERS: {
  ...
  if (error && typeof error === 'object' && 'name' in error) {
    ...
    if (error.name === 'ValidationError') {
      return getErrorMessage('VALIDATION', 'INVALID_FORMAT', {
        ...context,
        field: ('field' in error ? error.field as string : undefined) || 'input'
      });
    }
