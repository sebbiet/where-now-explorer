...
describe('RequestBatcherService', () => {
  ...
  describe('single request processing', () => {
    ...
    it('should handle single request with custom config', async () => {
      ...
      const requestPromise = batcher.batch(
        'custom-type',
        'request',
        mockProcessor
      );
  ...
  describe('request queuing', () => {
    ...
    it('should maintain separate queues for different types', async () => {
      ...
      // Act
      const geoPromise1 = batcher.batch(
        'geocode',
        'geo-req1',
        geocodeProcessor
      );
      const geoPromise2 = batcher.batch(
        'geocode',
        'geo-req2',
        geocodeProcessor
      );
      const revPromise1 = batcher.batch(
        'reverse-geocode',
        'rev-req1',
        reverseProcessor
      );
      const revPromise2 = batcher.batch(
        'reverse-geocode',
        'rev-req2',
        reverseProcessor
      );
  ...
  describe('error handling in batch', () => {
    ...
    it('should handle processor returning insufficient results', async () => {
      ...
      await expect(promise2).rejects.toThrow(
        'Batch processing returned insufficient results'
      );
    ...
    it('should handle mixed success/failure results', async () => {
      ...
      const mockProcessor = vi.fn().mockImplementation((requests) => {
        ...
        return Promise.resolve(
          requests.slice(0, Math.floor(requests.length / 2))
        );
      });
  ...
  describe('request deduplication in batch', () => {
    it('should handle duplicate requests in the same batch', async () => {
      ...
      expect(results).toEqual([
        'processed-same-request',
        'processed-same-request',
        'processed-different-request',
      ]);
    });
  });
