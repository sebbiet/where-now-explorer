...
export class OfflineModeService {
  ...
  cacheRoute(
    ...
    const existingIndex = this.offlineData.routes.findIndex(r => r.id === routeId);
    ...
    if (existingIndex >= 0) {
      ...
      // Enforce max size
      if (this.offlineData.routes.length > OfflineModeService.MAX_CACHED_ROUTES) {
        this.pruneOfflineData();
      }
    }
  ...
  cacheDestination(destination: GeocodeResult): void {
    ...
    if (!exists) {
      ...
      // Enforce max size
      if (this.offlineData.destinations.length > OfflineModeService.MAX_CACHED_DESTINATIONS) {
        // Remove oldest destinations
        this.offlineData.destinations = this.offlineData.destinations.slice(-OfflineModeService.MAX_CACHED_DESTINATIONS);
      }
    }
  }
   ...
   */
  private generateRouteId(origin: LocationData, destination: GeocodeResult): string {
    const originKey = `${origin.latitude?.toFixed(4)},${origin.longitude?.toFixed(4)}`;
    const destKey = destination.place_id || `${destination.lat},${destination.lon}`;
  ...
  private pruneOfflineData(): void {
    ...
    this.offlineData.routes = this.offlineData.routes.slice(0, OfflineModeService.MAX_CACHED_ROUTES);
