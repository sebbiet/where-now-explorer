...
describe('DestinationInput', () => {
  ...
  describe('input rendering', () => {
    it('should render input with correct placeholder', () => {
      ...
      const input = screen.getByPlaceholderText(
        'Start typing a place name, landmark or address'
      );
  ...
  describe('autocomplete suggestions display', () => {
    ...
    it('should show "Searching..." while loading', async () => {
      ...
      vi.mocked(GeocodingService.geocode).mockImplementation(
        () =>
          new Promise((resolve) =>
            setTimeout(() => resolve(mockGeocodeResults), 100)
          )
      );
  ...
  describe('loading state during search', () => {
    it('should show loading state while searching', async () => {
      ...
      vi.mocked(GeocodingService.geocode).mockReturnValue(
  ...
  describe('API error handling', () => {
    it('should handle API errors gracefully', async () => {
      ...
      const consoleErrorSpy = vi
        .spyOn(console, 'error')
        .mockImplementation(() => {});
      vi.mocked(GeocodingService.geocode).mockRejectedValue(
        new Error('API Error')
      );
      ...
      await waitFor(() => {
        expect(consoleErrorSpy).toHaveBeenCalledWith(
          'Error fetching place suggestions:',
          expect.any(Error)
        );
      });
